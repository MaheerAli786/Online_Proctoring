...
def start_recording(issue):
    global recording, record_start_time, video_writer, violation
    violation = issue
    if not recording:
        record_start_time = time.time()
        recording = True
        video_writer = cv2.VideoWriter(
            record_filename, cv2.VideoWriter_fourcc(*'mp4v'), 20.0, (640, 480)
        )


# Function to stop recording and save to DB
def stop_recording():
    global recording, video_writer, violation
    if recording:
        recording = False
        video_writer.release()  # Finish writing video

        # Read the recorded file as binary
        with open(record_filename, "rb") as f:
            video_blob = f.read()

        # Store in DB
        cursor.execute(
            "INSERT INTO violations (studentid,subjectid,assesmentid,video,violation) VALUES (%s,%s,%s,%s,%s)",
            (studentid, subjectid, assesmentid, video_blob, violation),
        )
        db.commit()
...
while cap.isOpened():
    ...
    if recording:
        video_writer.write(frame)  # Write the real-time frame directly
    ...
    if(z>0):
        ...
        print("Running Successfully", flush=True)  # Important: flush=True ensures immediate output
        z-=1
cap.release()
